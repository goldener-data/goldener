...
class TestGoldSplitter:
    ...
    def test_max_samples(self, descriptor, selector):
        """Test that max_samples limits the number of samples processed."""
        sets = [GoldSet(name="train", ratio=0.5), GoldSet(name="val", ratio=0.5)]
        splitter = GoldSplitter(
            sets=sets, 
            descriptor=descriptor, 
            selector=selector,
            max_samples=2  # Only process 2 samples
        )
        
        # Verify max_samples was set on descriptor and selector
        assert splitter.descriptor.max_samples == 2
        assert splitter.selector.max_samples == 2
        
        # Dataset with 10 items
        # With max_samples=2, only first 2 items should be processed
        splitted = splitter.split(
            dataset=DummyDataset(
                [{"data": torch.rand(3, 8, 8), "idx": idx} for idx in range(10)]
            )
