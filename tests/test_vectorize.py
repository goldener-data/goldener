import torch
import pytest
from goldener.vectorize import Vectorizer, Filter2DWithCount, FilterLocation


class TestVectorizer:
    def make_tensor(self, shape=(5, 3)):
        # Generate a random integer tensor of the given shape
    ...
    def test_vectorize_with_y(self):
        x = self.make_tensor()
        # y will mask out some, but not all, vectors (shape [5, 1])
        y = torch.zeros(5, 1)
        y[0] = 1
        y[2] = 1
        y[4] = 1
        v = Vectorizer()
        out = v.vectorize(x, y)
        expected = x[y.squeeze(-1).bool()]
        assert torch.equal(out, expected)

    def test_vectorize_with_y_mask(self):
        x = self.make_tensor()
        # y mask with shape (5, 1), select some rows
        y = torch.tensor([[1], [0], [1], [0], [1]], dtype=torch.bool)
        v = Vectorizer()
        out = v.vectorize(x, y)
        expected = x[y.squeeze(-1)]
        assert torch.equal(out, expected)

    def test_vectorize_y_broadcast(self):
        x = self.make_tensor((4, 3))
        y = torch.ones(1, 1)
        v = Vectorizer()
        out = v.vectorize(x, y)
        # y will be broadcast to (4, 1), so all rows are kept
        assert torch.equal(out, x)

    def test_vectorize_with_keep(self):
        x = self.make_tensor()
        # Test with Filter2DWithCount as keep
        keep = Filter2DWithCount(
            filter_count=2, filter_location=FilterLocation.START, keep=True
        )
        v = Vectorizer(keep=keep)
        out = v.vectorize(x)
        assert torch.equal(out, x[:2])

    def test_vectorize_with_remove(self):
        x = self.make_tensor()
        # Test with Filter2DWithCount as remove
        remove = Filter2DWithCount(
            filter_count=2, filter_location=FilterLocation.END, keep=False
        )
        v = Vectorizer(remove=remove)
        out = v.vectorize(x)
        assert torch.equal(out, x[:-2])
    ...
    def test_vectorize_with_keep_and_remove(self):
        x = self.make_tensor()
        keep = Filter2DWithCount(
            filter_count=4, filter_location=FilterLocation.START, keep=True
        )
        remove = Filter2DWithCount(
            filter_count=1, filter_location=FilterLocation.END, keep=False
        )
        v = Vectorizer(keep=keep, remove=remove)
        out = v.vectorize(x)
        assert torch.equal(out, x[:4][:-1])

    def test_vectorize_with_transform_y(self):
        x = self.make_tensor((5, 3))
        y = torch.randint(0, 10, (5, 1))

        def transform_y(y):
            # Only keep rows where y > 5
            return (y > 5).to(torch.int64)

        v = Vectorizer(transform_y=transform_y)
        out = v.vectorize(x, y)
        expected = x[transform_y(y).squeeze(-1).bool()]
        assert torch.equal(out, expected)
    ...
    def test_vectorize_3d_input(self):
    ...
    def test_vectorize_4d_input(self):
    ...
    def test_vectorizer_invalid_keep_type(self):
        with pytest.raises(ValueError):
            Vectorizer(keep=Filter2DWithCount())

    def test_vectorizer_invalid_remove_type(self):
        with pytest.raises(ValueError):
            Vectorizer(remove=Filter2DWithCount())
